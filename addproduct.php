...
<html lang="en">
  <head>
    ...
    <title>Raahi - Add New Products</title>
  ...
  <body class="violetgradient">
    ...
    $('#form1').on('submit', function(event) {
        ...
        web3.eth.getAccounts().then(async function(accounts) {
          ...
          .then(receipt => {
              ...
              sendWhatsappNotification(prodid);
          });
          //console.log(receipt);
        });
        $("#prodname").val('');
    ...
    function sendWhatsappNotification(prodname) {
  var apikey = "2938973"; // replace API_KEY with your WhatsApp API key
  var phone = "+917506210598"; // replace with the phone number you want to send the message to
  var message = "Item has been updated "+prodname; // message to be sent
  ...
  $.get(url, function(data) {
    ...
    $('#form2').on('submit', function(event) {
        event.preventDefault(); // to prevent page reload when form is submitted
        // prodid = $('#prodid').val();
        var prodid = Math.floor(Math.random() * 1000000);
        prodid = prodid.toString().padStart(6, '0');
        // prodid = $('#prodid').val(prodid);
